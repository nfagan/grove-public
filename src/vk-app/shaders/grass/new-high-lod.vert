...
vec2 calculate_wind_tip_displacement(vec2 world_pos_xz, vec4 wind_world_bound_xz, sampler2D wind_displacement_texture) {
...
float attenuate_toward_world_edge(vec2 world_uv) {
#if 1
  float d = length(world_uv * 2.0 - 1.0);
  float atten = 1.0 / 12.0;
  return d < (1.0 - atten) ? 1.0 : 0.0;
#else
  const float atten = 0.0781;
  float gt = float(any(greaterThan(world_uv, vec2(1.0 - atten))));
  float lt = float(any(lessThan(world_uv, vec2(atten))));
  return 1.0 - max(gt, lt);
#endif
}
...
void main() {
  ...
  */
  ...
  vs_out.v_y = y;
  vs_out.color_uv = vec2(compute_splotch(world_uv), 0.0);

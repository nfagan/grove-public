...
#if HAS_SWELL_FRAC
float compute_swell_frac(float y, float y0, float y1, vec3 instance_pos, vec3 child_instance_pos, vec2 swell_info) {
  const float crng = 0.1;
  const float strength_scale = 0.2;

  float yf = y_position01(mix(instance_pos, child_instance_pos, y), y0, y1);
...
#if HAS_SWELL_FRAC
  float swell_frac = compute_swell_frac(
    y, aabb_p0().y, aabb_p1.y, instance_position, child_instance_position, push_constants.swell_info.xy);

...
struct RenderInstanceGroup {
  uvec4 alpha_image_color_image_indices_uv_offset_color_image_mix_unused;
...
layout (location = 6) out vec2 v_color_image1_index_color_mix;
#endif  //  ENABLE_COLOR_IMAGE_MIX

#endif  //  USE_ARRAY_IMAGES
...
float get_uv_offset(in RenderInstanceGroup inst_group) {
  ...
  return uintBitsToFloat(inst_group.alpha_image_color_image_indices_uv_offset_color_image_mix_unused.y);
}

float get_alpha_image_index(in RenderInstanceGroup inst_group) {
  return float(inst_group.alpha_image_color_image_indices_uv_offset_color_image_mix_unused.x & 0xffffu);
}

float get_color_image0_index(in RenderInstanceGroup inst_group) {
  return float((inst_group.alpha_image_color_image_indices_uv_offset_color_image_mix_unused.x >> 16u) & 0xffu);
}

#ifdef ENABLE_COLOR_IMAGE_MIX
float get_color_image1_index(in RenderInstanceGroup inst_group) {
  return float((inst_group.alpha_image_color_image_indices_uv_offset_color_image_mix_unused.x >> 24u) & 0xffu);
}
float get_color_image_mix(in RenderInstanceGroup inst_group) {
  return uintBitsToFloat(inst_group.alpha_image_color_image_indices_uv_offset_color_image_mix_unused.z);
}
vec2 get_color_image_mix_info(in RenderInstanceGroup inst_group) {
  return vec2(get_color_image1_index(inst_group), get_color_image_mix(inst_group));
}
#endif
...
vec3 wind_displacement(in RenderInstance inst, vec3 aabb_p0, vec3 aabb_p1,
                       vec3 world_center, vec3 inst_trans, float t) {
  vec2 world_center_xz = world_center.xz;
  vec2 sampled_wind = sample_wind_tip_displacement(
    world_center_xz, wind_world_bound_xz, wind_displacement_texture);
  float wind_atten = wind_attenuation(
    sampled_wind,
    wind_displacement_limits_wind_strength_limits.xy,
    wind_displacement_limits_wind_strength_limits.zw);
  ...
  unpack_axis_root_info(inst.wind_node_info0, root_info0, child_root_info0);
  unpack_axis_root_info(inst.wind_node_info1, root_info1, child_root_info1);
  unpack_axis_root_info(inst.wind_node_info2, root_info2, child_root_info2);

  vec3 displaced = wind_displacement(
    inst_trans, t, aabb_p0, aabb_p1.xyz, root_info0, root_info1, root_info2, wind_atten);
...
void main() {
  ...
  v_color_image_index = get_color_image0_index(inst_group);

  #ifdef ENABLE_COLOR_IMAGE_MIX
  v_color_image1_index_color_mix = get_color_image_mix_info(inst_group);
  #endif
